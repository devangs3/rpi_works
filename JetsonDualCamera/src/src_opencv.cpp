...
{
public:
	explicit JetsonCameraStream(int sensor_id, FrameBuffer &buffer) : sensor_id_(sensor_id), buffer_(buffer), running_(false) {}
...
private:
	void capture_loop()
	{
		// The correct, native C++ format structure
		std::string pipeline =
		...
		while (running_)
		{
			...
			else
			{
				...
				std::cout << "[Camera Thread] Failed to read frame or stream stopped " << sensor_id_ << std::endl;
...
int main()
{
	...
	std::cout << "[System] Force resetting nvargus-daemon to clear stuck ports..." << std::endl;

	// 1. Trigger the systemctl restart command via shell execution [1]
	int result = std::system("sudo systemctl restart nvargus-daemon");

	if (result != 0)
	{
		std::cerr << "[Warning] Failed to restart daemon. Make sure you ran this app with sudo!" << std::endl;
	}

	// 2. CRITICAL: Give the Linux operating system 1.5 to 2 full seconds
	// to tear down and fully reboot the service before hitting it with your pipelines.
	std::this_thread::sleep_for(std::chrono::milliseconds(2000));

	std::cout << "[System] Camera service healthy. Starting stream allocation..." << std::endl;
	...
	while (true)
	{
		...
		// If any camera produces frames, increment the counter and show latest frames
		if (got0 || got1)
		{
			// Stack frames horizontally for a ultra-fast unified window layout
			// sometimes one frame might be empty, one might not
			// ensure uniform size before concat
			if ((view0.rows == view1.rows) && (view0.type() == view1.type()))
			{
		...
		{
			// Assume none updated, so sleep until the camera frames arrive
			std::this_thread::sleep_for(std::chrono::milliseconds(1));
		}
		...
		// Yield control to threads to grab data
